...
public class ProductRepository implements IProductRepository {
    ...
    private static final String SELECT_PRODUCT_BY_FEATURE = "select * from product where `product_status` = ?";
    ...
    public Product getProductFeature(String nameProductFeature) {
        ...
        if (connection != null) {
            try {
                preparedStatement = connection.prepareStatement(SELECT_PRODUCT_BY_FEATURE);
                preparedStatement.setString(1, nameProductFeature);
                resultSet = preparedStatement.executeQuery();
                while (resultSet.next()){
                    int productId = resultSet.getInt("product_id");
                    String productName = resultSet.getString("product_name");
